  ...
  <>
    <section className="flex flex-col w-full mt-10">
      <div className="flex flex-row items-center">
        ...
        <h2 className="ml-2 text-3xl font-bold ">{title}</h2>
      </div>
      <ul className="relative z-5">
        {expContents.map((i, index) => (
          <li key={`education-${index}`} className={`relative pl-10 mb-0}`}>
            <div
              className={`absolute left-5 right-0 bottom-0 top-0 w-px bg-gray-300 h-full ${
                index === expContents.length - 1 ? "h-0" : ""
              }
                ${index === 0 ? "top-8" : ""}`}
            ></div>
            <div className="absolute w-4 h-4 bg-blue-100 rounded-full left-3 top-7"></div>
            <div className="absolute w-2 h-2 bg-blue-500 rounded-full left-4 top-8"></div>
            <ExpContent
              key={`exp-education-${i}`}
              position={i.position}
              duration={i.duration}
